...
describe('cmdShowCpu', () => {
    ...
    describe('analyzer', () => {
        it('should parse valid CPU utilization data and return the expected object', () => {
            const data =
                'CPU utilization for one second: 14%; one minute: 12%; five minutes: 12% CPU MAX utilization: 92%(2023-10-5 17:21:8)';
        ...
        it('should throw an error when provided with invalid CPU utilization data', () => {
            ...
            expect(() => cmdShowCpu.analyzer(data)).toThrow(
                'Invalid cpu information received'
            );
        });
    });
});
