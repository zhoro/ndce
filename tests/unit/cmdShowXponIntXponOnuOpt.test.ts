...
describe('cmdShowXponIntXponOnuOpt', () => {
    it('should return correct command with default board number', () => {
        const result: IDeviceCommand<IBdcomOnuInterfaceXponOpticalDetails> = cmdShowXponIntXponOnuOpt(undefined, 1, 1);
        expect(result.command()).to.equal('show gpon int gpon0/1:1 onu optical-transceiver-diagnosis');
    });

    it('should return correct command with provided board number', () => {
        const result: IDeviceCommand<IBdcomOnuInterfaceXponOpticalDetails> = cmdShowXponIntXponOnuOpt(2, 1, 1);
        expect(result.command()).to.equal('show gpon int gpon2/1:1 onu optical-transceiver-diagnosis');
    });

    it('should return correct onu details when data is provided', () => {
        const result: IDeviceCommand<IBdcomOnuInterfaceXponOpticalDetails> = cmdShowXponIntXponOnuOpt(0, 1, 1);
        const data = '-----\n' +
