 ...
 */
export async function exCmdShEponInactiveOnu(
    ...
    try {
        ...
        for (const networkDevice of networkDevs) {
            ...
            if (device.isLogged) {
                ...
                await Promise.all(
                    inactiveOnus.map(async (inactiveOnu) => {
    ...
    async function processInactiveOnu(
        inactiveOnu: IBdcomInactiveOnu,
        deviceId: number
    ) {
        let lastRegister =
            inactiveOnu.xponType === 'epon'
                ? new Date(
                      `${inactiveOnu.lastRegDate} ${inactiveOnu.lastRegTime}`
                  )
                : new Date(0);
        let lastDeregister = new Date(
            `${inactiveOnu.lastDeregDate} ${inactiveOnu.lastDeregTime}`
        );
