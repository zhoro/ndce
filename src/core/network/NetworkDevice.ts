...
export class NetworkDevice {
    ...
    constructor(
        ...
        public managementAccess: DeviceManagementAccess
    ) {
    ...
    async login() {
        try {
            if (this.transport.isConnected) {
                const result = await this.transport.login(
                    this.device.configuration.messageAuthFailed,
                    this.device.configuration.messageLoginPrompt
                );
    ...
    async execute(cmd: IDeviceCommand<any>) {
        try {
            if (this.transport.isConnected && this.transport.isLogged) {
                ...
                console.error(
                    'NetworkDevice.execute: device is not connected/authorized'
                );
                return Promise.reject();
            }
        } catch (e) {
        ...
        try {
            if (this.transport.isConnected) {
                await this.transport
                    ...
                    .catch((e) => {
                        console.error(
                            'NetworkDevice.disconnect exception: ' + e
                        );
                    })
                    .finally(() => {
