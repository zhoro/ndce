...
export const cmdShowXponInactiveOnu: IDeviceCommand<IBdcomInactiveOnu> = {
    ...
    analyzer: (data) => {
        ...
        const regex =
            /(EPON(?<board>\d{0,3})\/(?<port>\d{0,3}):(?<iface>\d{0,3})(\s+)?(?<mac>(?:[A-Fa-f0-9]{2}[:-]){5}[A-Fa-f0-9]{2}|(?:[A-Fa-f0-9]{4}\.){2}[A-Fa-f0-9]{3,4})\s+(?<dreason>\S+)\s+(?<datereg>\S+)(\s|\.)(?<treg>\S+)\s(?<ddereg>\S+)(\s|\.)(?<tdereg>\S+)\s(?<deregreason>\S+)\s+(?<adays>\d+)(\s+)?(\.)(?<atime>\d{2}:\d{2}:\d{2}))/gm;
