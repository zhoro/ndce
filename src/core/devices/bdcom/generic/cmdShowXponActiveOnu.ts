...
export const cmdShowXponActiveOnu: IDeviceCommand<IBdcomActiveOnu> = {
    ...
    analyzer: (data) => {
        ...
        const regex =
            /(EPON(?<board>\d)\/(?<port>\d):(?<iface>\d{0,3})\s+(?<mac>\S+)\s+(?<status>\S+)\s+(?<oamstatus>\S+)\s+(?<distance>\d+)\s+(?<rtt>\d+)\s+(?<datereg>\S+)(\.|\s)(?<treg>\d{2}\:\d{2}\:\d{2})\s+(?<datedereg>\S+)(\.|\s)(?<tdereg>\d{2}\:\d{2}\:\d{2})\s+(?<deregreason>\S+)\s+(?<alivedays>\d+)(\s+\.|\.)(?<alivetime>\d{2}\:\d{2}\:\d{2}))/gm;
