...
export const cmdShowXponInactiveOnu: IDeviceCommand<IBdcomInactiveOnu> = {
    ...
    analyzer: (data) => {
        ...
        const regex = /GPON(\d)\/(\d):(\d{0,3})\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)/gm
        const deregisteredONUs: any[] = [];
        let match
        while ((match = regex.exec(input)) !== null) {
            const [wholeMatch, xponBoard, xponPort, xponInterface, serialNumber, loid, status, lastDeregDate, lastDeregTime, lastDeregReason] = match;
